...
internal open class MutableBigInteger {
    ...
    private val magnitudeArray: IntArray
        get() = if (offset > 0 || value.size != intLen)
            value.copyOfRange(offset, offset + intLen) else value
    ...
    private val lowestSetBit: Int
        get() {
            ...
            b = value[j + offset]
            return if (b == 0) -1 else (intLen - 1 - j shl 5) + Integers.numberOfTrailingZeros(b)
        }
    ...
    fun toCompactValue(sign: Int): Long {
        ...
        val v = if (len == 2)
            mag[1].toLong() and LONG_MASK or (d and LONG_MASK shl 32)
        else
            d and LONG_MASK
    ...
    fun copyValue(src: MutableBigInteger) {
        ...
        if (value.size < len)
            value = IntArray(len)
        Arrays.arraycopy(src.value, src.offset, value, 0, len)
        intLen = len
        offset = 0
    }
    ...
    fun copyValue(`val`: IntArray) {
        ...
        if (value.size < len)
            value = IntArray(len)
        Arrays.arraycopy(`val`, 0, value, 0, len)
        intLen = len
        offset = 0
    }
    ...
    fun add(addend: MutableBigInteger) {
        ...
        if (carry > 0) { // Result must grow in length
            ...
            if (result.size < resultLen) {
                ...
                Arrays.arraycopy(result, 0, temp, 1, result.size)
                temp[0] = 1
                result = temp
            } else {
                result[rstart--] = 1
    ...
    fun addDisjoint(addend: MutableBigInteger?, n: Int) {
        ...
        Arrays.arraycopy(value, offset, result, rstart + 1 - x, x)
        y -= x
        rstart -= x

        val len = min(y, addend.value.size - addend.offset)
        Arrays.arraycopy(addend.value, addend.offset, result, rstart + 1 - y, len)
    ...
    fun divideOneWord(divisor: Int, quotient: MutableBigInteger): Int {
        ...
        if (intLen == 1) {
            ...
            val q = dividendValue / divisorLong
            val r = dividendValue - q * divisorLong
            quotient.value[0] = q.toInt()
            quotient.intLen = if (q == 0L) 0 else 1
            quotient.offset = 0
            return r.toInt()
        }
    ...
    fun divideKnuth(b: MutableBigInteger, quotient: MutableBigInteger, needRemainder: Boolean = true): MutableBigInteger? {
        var b = b
        if (b.intLen == 0)
            throw IllegalArgumentException("BigInteger divide by zero")
    ...
    fun divideAndRemainderBurnikelZiegler(b: MutableBigInteger, quotient: MutableBigInteger): MutableBigInteger {
        ...
        if (r < s) {
            ...
            val m = 1 shl 32 - Integers.numberOfLeadingZeros(s / BigInteger.BURNIKEL_ZIEGLER_THRESHOLD)
     ...
     */
    fun bitLength(): Long {
        return if (intLen == 0) 0 else intLen * 32L - Integers.numberOfLeadingZeros(value[offset])
    }
    ...
    private fun divideMagnitude(div: MutableBigInteger,
        ...
        if (shift > 0) {
            ...
            Arrays.arraycopy(value, offset, rem.value, 1, intLen)
            rem.intLen = intLen
            rem.offset = 1
        }
    ...
    private fun divideLongMagnitude(ldivisor: Long, quotient: MutableBigInteger): MutableBigInteger {
        ...
        // D1 normalize the divisor
        val shift = Longs.numberOfLeadingZeros(ldivisor)
        if (shift > 0) {
    ...
    fun mutableModInverse(p: MutableBigInteger): MutableBigInteger? {
        ...
        if (isEven)
            throw IllegalArgumentException("BigInteger not invertible.")
    ...
    fun modInverseMP2(k: Int): MutableBigInteger {
        if (isEven)
            throw IllegalArgumentException("Non-invertible. (GCD != 1)")
